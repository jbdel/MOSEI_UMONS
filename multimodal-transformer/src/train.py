...
def train_model(settings, hyp_params, train_loader, valid_loader, test_loader):
    ...
    def evaluate(model, ctc_a2l_module, ctc_v2l_module, criterion, test=False):
        ...
        with torch.no_grad():
            for i_batch, (batch_X, batch_Y, batch_META) in enumerate(loader):
                ...
                print([batch_META[0][0], batch_META[1][0], batch_META[2][0]])
                for i in range(batch_size):
                    if abs(preds[i] - eval_attr[i]) > 2.0:
                        err_examples.append([batch_META[0][0], batch_META[1][0], batch_META[2][0], str(preds[i].item()), str(eval_attr[i].item())])
        ...
        print("number of error examples: ",len(err_examples))
        with open("error_example.txt", "w") as f:
            for item in err_examples:
                f.write(" ".join(item)+"\n")
    ...
    for epoch in range(1, hyp_params.num_epochs+1):
        ...
        if val_loss < best_valid:
            ...
            best_valid = val_loss
    '''
    print("model name", hyp_params.name)
    model = load_model(hyp_params, name=hyp_params.name)
    _, results, truths = evaluate(model, ctc_a2l_module, ctc_v2l_module, criterion)
