...
class SA(nn.Module):
    ...
    def forward(self, y, y_mask):
        att_y, att_map = self.mhatt(y, y, y, y_mask)
        y = self.norm1(y + self.dropout1(att_y))
...
class SGA(nn.Module):
    ...
    def forward(self, x, y, x_mask, y_mask):
        att_x, att_map = self.mhatt1(v=x, k=x, q=x, mask=x_mask)
        x = self.norm1(x + self.dropout1(att_x))

        att_x, att_map = self.mhatt2(v=y, k=y, q=x, mask=y_mask)
        x = self.norm2(x + self.dropout2(att_x))
...
class Block(nn.Module):
    ...
    def forward(self, x, x_mask, y, y_mask, z, z_mask):
        ...
        ay, att_map_y = self.sa2(y, x, y_mask, x_mask)
        az, att_map_z = self.sa3(z, x, z_mask, x_mask)
        ...
        if self.last:
            return x, y, z, att_map_y, att_map_z
...
class Model_LAV(nn.Module):
    ...
    def forward(self, x, y, z):
        ...
        for i, dec in enumerate(self.enc_list):
            ...
            x, y, z, att_map_y, att_map_z = dec(x, x_m, y, y_m, z, z_m)
        ...
        return ans, att_map_y, att_map_z